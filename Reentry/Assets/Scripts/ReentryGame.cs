...
public sealed class ReentryGame:MonoBehaviour {
...
#endif
#if !UNITY_WEBGL
 if(Array.IndexOf(Environment.GetCommandLineArgs(),"--landing-qa")>=0){qa=true;landingQa=true;qaStage=90;Begin();flight.Height=500.1;flight.Velocity=80;flight.Downrange=flight.Destination-2900;flight.Gamma=-8*Math.PI/180;flight.Alpha=8*Math.PI/180;flight.Temperature=650;flight.Step(false,.05);Finish();}
#endif
#if !UNITY_WEBGL
 if(Array.IndexOf(Environment.GetCommandLineArgs(),"--failure-qa")>=0){qa=true;qaStage=98;Begin();flight.Temperature=2400;flight.Step(false,.05);Finish();Invoke(nameof(CaptureFailureQA),4);}
 if(Array.IndexOf(Environment.GetCommandLineArgs(),"--reduced-motion")>=0)world.ReduceMotion=true;
#endif
 Debug.Log("EMBER_PHYSICAL_READY / model forward -Z, world metres and kilometres calibrated");
 }
 void Begin(){CancelInvoke(nameof(PresentDebrief));flight=new FlightModel(mission);running=true;paused=false;accumulator=0;forecastClock=0;previousHeld=false;warningClock=0;qaHeld=true;hud.Menu.SetActive(false);hud.Debrief.SetActive(false);hud.Flight.SetActive(true);hud.PauseButton.gameObject.SetActive(true);hud.PausePanel.SetActive(false);hud.ResetPlot();UpdateForecast();hud.UpdateFlight(flight,false,low,high,current);sound.Tone(true);Debug.Log("MISSION_START "+mission);}
 ...
 void Update(){if(world==null)return;float dt=Mathf.Min(Time.deltaTime,.1f);hud.TickLayout();hud.TickDebrief(dt,world.ReduceMotion);
  ...
  world.Tick(flight,running||qaStage==99,flight!=null&&flight.Finished,paused?0:dt);if(flight!=null){hud.SetDestination(world.RunwayViewport,world.DisplayRange,world.DisplayAltitude);if(flight.Outcome==FlightOutcome.Landed&&hud.Flight.activeSelf)hud.UpdateAutoland(world.DisplayAltitude,world.DisplayRange,world.LandingState);}sound.Tick(flight==null?0:Mathf.Clamp01((float)flight.HeatFlux/1700000),running&&!paused);
  sound.Landing(flight!=null&&flight.Outcome==FlightOutcome.Landed?(LandingFrame?)world.LandingState:null);
  if(flight!=null&&flight.Outcome==FlightOutcome.Landed&&world.LandingComplete&&!hud.Debrief.activeSelf)PresentDebrief();
  if(landingQa){var stage=world.LandingState.Stage;if(qaStage==90&&stage==LandingStage.Flare){Capture("flare");qaStage++;}else if(qaStage==91&&stage==LandingStage.Touchdown){Capture("touchdown");qaStage++;}else if(qaStage==92&&stage==LandingStage.Rollout){Capture("rollout");qaStage++;}else if(qaStage==93&&stage==LandingStage.Stopped){Capture("stopped");qaStage++;Invoke(nameof(QuitQA),5);}}
 ...
 void Finish(){running=false;bool success=flight.Outcome==FlightOutcome.Landed;newBest=success&&flight.Score>PlayerPrefs.GetInt("EmberBestV2",0);hud.PauseButton.gameObject.SetActive(false);hud.Status.text=success?"AUTOLAND CAPTURE\n<size=16>Approach energy within limits</size>":"VEHICLE LOST";hud.Instruction.text=success?"Landing system engaged. Runway acquired.":"Flight recorder saved.";if(!success)world.Burst();sound.Tone(success);if(!success)Invoke(nameof(PresentDebrief),2.5f);
 ...
 void PresentDebrief(){hud.ShowDebrief(flight,newBest,world.ReduceMotion);if(flight.Outcome==FlightOutcome.Landed)sound.Victory();Debug.Log("DEBRIEF_PRESENTED "+flight.Outcome);if(landingQa)Invoke(nameof(CaptureVictoryQA),2);}
 void CaptureVictoryQA(){hud.LogDebrief();Capture(world.ReduceMotion?"victory-reduced":"victory");}
 void CaptureFailureQA(){Capture("failure");Invoke(nameof(QuitQA),1);}
