...
public sealed class ReentryWorld {
 ...
 public Transform Ship {get;private set;}Transform planet,runway,clouds;
 Transform envelope; Material gas; Transform gear;ParticleSystem sparks;Light sun,heatLight;
 public bool ReduceMotion;float clock,endClock;bool lastEnd;
 ...
 static readonly Vector3 NoseForward=Vector3.back;
 ...
 public ReentryWorld(){
  ...
  var shell=new GameObject("Continuous bow shock and plasma sheath",typeof(MeshFilter),typeof(MeshRenderer));shell.layer=ShipLayer;envelope=shell.transform;gas=Mat("Plasma",new Color(1,.3f,.06f,1));shell.GetComponent<MeshFilter>().sharedMesh=PlasmaMesh();shell.GetComponent<MeshRenderer>().sharedMaterial=gas;
  foreach(var t in Ship.GetComponentsInChildren<Transform>())if(t.name=="LandingGear")gear=t;
 ...
 public void Tick(FlightModel f,bool playing,bool ended,float dt){
  ...
  Vector3 offset=(playing||ended)?Vector3.Lerp(new Vector3(9,5,16),new Vector3(2.5f,3,15),Mathf.Clamp01(1-h/10)):new Vector3(10,5,-15);
  ...
  environment.transform.position=new Vector3(offset.x*.001f,h+offset.y*.001f,offset.z*.001f);environment.transform.rotation=Camera.transform.rotation;environment.fieldOfView=Camera.fieldOfView;
  ...
  float angle=range/Radius;runway.position=new Vector3(0,Radius*(Mathf.Cos(angle)-1),-Radius*Mathf.Sin(angle));runway.rotation=Quaternion.Euler(-angle*Mathf.Rad2Deg,0,0);
  planet.rotation=Quaternion.AngleAxis((float)(f?.Downrange??0)/1000/Radius*Mathf.Rad2Deg,Vector3.right)*Quaternion.Euler(0,75,70);clouds.rotation=planet.rotation;var earth=planet.GetComponent<Renderer>().sharedMaterial;earth.SetFloat("_Altitude",h);
  Quaternion flightRotation=Quaternion.Euler(gamma,0,0);Vector3 flow=flightRotation*Vector3.forward;Vector3 origin=Ship.TransformPoint(new Vector3(0,-.13f,-3.8f));
  heatLight.transform.position=origin+Vector3.up*.3f;heatLight.intensity=playing?flux*10:0;
  envelope.position=origin;envelope.rotation=flightRotation;gas.SetFloat("_Heat",playing?flux:0);envelope.gameObject.SetActive(playing&&flux>.02f);
  if(gear)gear.localScale=new Vector3(1,Mathf.SmoothStep(.001f,1,Mathf.Clamp01((1500-h*1000)/1000)),1);
 ...
 public static Mesh PlasmaMesh(){
  const int rings=48,sides=64;var v=new Vector3[(rings+1)*(sides+1)];var uv=new Vector2[v.Length];var tris=new int[rings*sides*6];int k=0;
  for(int r=0;r<=rings;r++){float u=(float)r/rings;float width=.08f+4.8f*Mathf.Sqrt(1-Mathf.Exp(-u*12));for(int j=0;j<=sides;j++){float a=j*Mathf.PI*2/sides;int i=r*(sides+1)+j;v[i]=new Vector3(Mathf.Cos(a)*width,Mathf.Sin(a)*width*.38f,u*23-.35f);uv[i]=new Vector2(u,(float)j/sides);if(r<rings&&j<sides){tris[k++]=i;tris[k++]=i+sides+1;tris[k++]=i+1;tris[k++]=i+1;tris[k++]=i+sides+1;tris[k++]=i+sides+2;}}}
  var m=new Mesh{name="Smooth continuous shock envelope"};m.vertices=v;m.uv=uv;m.triangles=tris;m.RecalculateNormals();m.RecalculateBounds();return m;
 }
