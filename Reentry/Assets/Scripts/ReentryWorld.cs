...
public sealed class ReentryWorld {
 ...
 Material skyMaterial;VehicleAnimation vehicle;LandingEffects landingEffects;EnvironmentLighting lighting;LandingSequence landing;LandingStage lastLandingStage;public LandingFrame LandingState;public bool LandingComplete=>landing!=null&&endClock>=landing.Duration+1.5;public float DisplayAltitude,DisplayRange,LandingProgress;ParticleSystem sparks;Light sun,heatLight;
 ...
 public ReentryWorld(){
  ...
  lighting=new EnvironmentLighting();
  ...
  vehicle=new VehicleAnimation(Ship);landingEffects=new LandingEffects();
 ...
 void Rematerial(Transform root,bool site){foreach(var r in root.GetComponentsInChildren<Renderer>()){if(site&&r.name=="Reclaimed coastal base"){r.enabled=false;continue;}var source=r.sharedMaterials;for(int i=0;i<source.Length;i++){string n=source[i]?source[i].name:"";Color c=new Color(.52f,.57f,.61f);if(site){c=new Color(.18f,.2f,.17f);if(n.Contains("Asphalt"))c=new Color(.035f,.04f,.05f);if(n.Contains("Concrete"))c=new Color(.2f,.22f,.24f);if(n.Contains("Markings"))c=new Color(.7f,.72f,.67f);if(n.Contains("Glass"))c=new Color(.04f,.15f,.2f);if(n.Contains("Metal"))c=new Color(.3f,.34f,.38f);if(n.Contains("Lights"))c=new Color(.1f,.4f,.75f);}else{if(n.Contains("Carbon"))c=new Color(.018f,.025f,.035f);if(n.Contains("Cockpit"))c=new Color(.075f,.13f,.18f);if(n.Contains("Titanium"))c=new Color(.42f,.46f,.49f);if(n.Contains("Orange"))c=new Color(.8f,.18f,.025f);if(n.Contains("Cyan"))c=new Color(.06f,.7f,1);}bool emissive=n.Contains("Lights")||n.Contains("Cyan");var m=Mat(emissive?"Unlit":"Lit",emissive?c*2:c);if(!site){m.SetFloat("_Smoothness",n.Contains("Cockpit")?.96f:n.Contains("Titanium")?.68f:.32f);m.SetFloat("_Metallic",n.Contains("Titanium")?.82f:n.Contains("Cockpit")?.32f:.02f);}if(!site&&(n.Contains("Ceramic")||n.Contains("Carbon"))){Object.Destroy(m);m=new Material(Resources.Load<Material>(n.Contains("Carbon")?"CarbonPBR":"HullPBR"));}source[i]=m;}r.sharedMaterials=source;}}
 public void Tick(FlightModel f,bool playing,bool ended,float dt){
  clock+=dt;if(ended)endClock+=dt;else {endClock=0;landing=null;}
  ...
  if(landed){if(landing==null){landing=new LandingSequence(range*1000,h*1000,(float)f.Velocity,gamma+alpha);endClock=0;lastLandingStage=LandingStage.Approach;Debug.Log("LANDING_CAPTURE / staged approach and rollout");}LandingState=landing.Evaluate(endClock);h=(float)LandingState.Height/1000;range=(float)LandingState.Range/1000;gamma=0;alpha=(float)LandingState.Pitch;if(LandingState.Stage!=lastLandingStage){Debug.Log("LANDING_STAGE "+LandingState.Stage+" / speed="+LandingState.Speed.ToString("F1")+" range="+LandingState.Range.ToString("F1"));lastLandingStage=LandingState.Stage;}}
  DisplayAltitude=h;DisplayRange=range;LandingProgress=landed?Mathf.Clamp01(endClock/(float)landing.Duration):0;
  Ship.position=Vector3.zero;Ship.rotation=Quaternion.Euler(gamma+alpha,0,ReduceMotion||landed?0:Mathf.Sin(clock*.8f)*.35f);
  Vector3 offset=(playing||ended)?Vector3.Lerp(new Vector3(9,5,16),new Vector3(0,3,15),Mathf.Clamp01(1-h/10)):new Vector3(10,5,-15);
  if(landed&&!ReduceMotion){float shot=Mathf.SmoothStep(0,1,Mathf.Clamp01((endClock-(float)landing.ApproachSeconds+2)/4));offset=Vector3.Lerp(offset,new Vector3(10,3,10),shot);}
  ...
  float sky=Mathf.Clamp01((35-h)/35);environment.backgroundColor=Color.Lerp(new Color(.001f,.003f,.01f),new Color(.19f,.39f,.59f),sky);sun.intensity=f?.Mission==2?1.3f:1.85f;lighting.Tick(h);
  ...
  landingEffects.Tick(h*1000,range,landed?(LandingFrame?)LandingState:null);
  vehicle.Tick(h*1000,f?.Attack??10,gamma+alpha,dt,landed?(LandingFrame?)LandingState:null);
