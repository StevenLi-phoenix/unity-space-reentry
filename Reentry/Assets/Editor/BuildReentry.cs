...
public static class BuildReentry {
 public static void Test(){FlightTests.Run();}
 ...
 static void Build(BuildTarget target,string output){
  ...
  foreach(string shader in new[]{"Universal Render Pipeline/Lit","Universal Render Pipeline/Unlit","Ember/Atmosphere","Ember/Planet","Ember/Plasma"}){string p="Assets/Resources/"+shader.Substring(shader.LastIndexOf('/')+1)+".mat";if(!AssetDatabase.LoadAssetAtPath<Material>(p))AssetDatabase.CreateAsset(new Material(Shader.Find(shader)),p);}
